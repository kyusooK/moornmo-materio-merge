<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            수주정보 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            수주정보
        </v-card-title >        
        ...
        <v-card-text>
            <Number label="SalesOrderNumber" v-model="value.salesOrderNumber" :editMode="editMode" :inputUI="''"/>
	...
	async created() {
        ...
        methods: {
            ...
            async produce(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['produce'].href), params)
